class User {
    ...
    public async subscribe(
        ...
        if (this.hasCard) {
            ...
            let urlForPaymentInAcquiring = await this.makeFirstOrder_(
                subscription, successRedirectURL, failureRedirectURL
            );

            SubscriptionRepository.add(subscription);
            return urlForPaymentInAcquiring;
        }
    }


    public async restoreSubscriptionOrSetAmount(
    ...
    private async makeFirstOrder_(
        ...
        let urlForPaymentInAcquiring = await order.makePaymentInAcquiring(
            successRedirectURL, failureRedirectURL
        );
